<template>
      <el-aside width="220px" class="image-aside" v-loading="loading">
        <div class="top">
            <AsidList :active="activeID == item.id" v-for="(item,index) in list" :key="index" @edit= "handleEdit(item)" @delete="handleDelete(item.id)" @click = handleChangeActiveID(item.id)>
                {{ item.name }}
            </AsidList>
        </div>
        <div class="bottom">
...
import AsidList from '~/components/AsidList.vue';
...
const handleSubmit = ()=>{
  formRef.value.validate((validate)=>{
    ...
    updateImageClass(editID.value,form) : createImageClass(form)
    fun.then(res=>{
